...
@pytest.mark.asyncio
async def test_hol_session_stale_data():
    """Test that stale data in buffer is handled correctly.

    Simulates interrupt scenario where buffer has stale response.
    """
    async with HOLSession(str(FIXTURES_DIR)) as session:
        result = await session.send('42;', timeout=10)
        assert '42' in result

        # Inject stale data (simulates interrupt scenario)
        session._buffer = b'stale_data\0'

        # Next send should return current response, not stale
        result = await session.send('99;', timeout=10)
        assert '99' in result
        assert 'stale' not in result


@pytest.mark.asyncio
async def test_hol_session_buffer_edge_cases():
    """Test _read_to_last_null handles various buffer states.

    Edge cases:
    - \\0\\0\\0 → pure nulls, output unchanged
    - seg\\0 → single segment
    - stale\\0current\\0 → returns current (last)
    - seg\\0trailing → keeps trailing incomplete data
    """
    async with HOLSession(str(FIXTURES_DIR)) as session:
        # Case 1: pure nulls - should return actual response
        session._buffer = b'\0\0\0'
        result = await session.send('1;', timeout=10)
        assert '1' in result

        # Case 2: single complete segment in buffer
        session._buffer = b'injected\0'
        result = await session.send('2;', timeout=10)
        assert '2' in result
        assert 'injected' not in result

        # Case 3: stale + current (two complete segments)
        session._buffer = b'old\0'
        result = await session.send('3;', timeout=10)
        assert '3' in result
        assert 'old' not in result

        # Case 4: trailing incomplete data should be preserved
        # After a normal send, buffer should be empty
        result = await session.send('4;', timeout=10)
        assert '4' in result
