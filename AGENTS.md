...
**Workflow:** `init → (prove until no goals → complete → splice → reenter) × N`

1. `initialize(file)` - Parse file, find first cheat
2. `start_current()` - Clear proof state, enter goaltree for current theorem
3. `complete_and_advance()` - Extracts proof via p(), returns it. Agent must splice into file, then call `start_current()` (via `hol_cursor_reenter`) for next theorem.
